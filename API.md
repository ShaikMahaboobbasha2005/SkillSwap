...
- Client events:
  ...
  - `mark_messages_read` (`{ swapId, messageIds }`, `callback`): Marks specified incoming unread messages in `swapId` as `read` in MongoDB and broadcasts `messages_status_update` to room `swap:<swapId>`.
- Server events:
