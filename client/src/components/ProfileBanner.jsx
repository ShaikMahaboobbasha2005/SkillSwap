...
export default function ProfileBanner({
  ...
  return (
    <div className="relative w-full h-36 sm:h-44 lg:h-48 rounded-t-2xl overflow-hidden group border-b border-[#E6E3DA]/60 dark:border-[#2A2E29]/60 bg-[#F7F6F2] dark:bg-[#202520]">
      {/* Hidden File Input */}
      {isOwner && (
        <input
          type="file"
      ...
      {bannerUrl ? (
        ...
        <div className="w-full h-full bg-gradient-to-r from-[#1B4332]/20 dark:from-[#1C2E24]/60 via-[#E4EEE8]/70 dark:via-[#202520] to-[#F7F6F2] dark:to-[#181B18] flex items-center justify-center relative">
      ...
      {/* Badge Indicator */}
      <div className="absolute top-3 left-4 sm:left-6 z-10 bg-white/85 dark:bg-[#181B18]/85 backdrop-blur-xs px-3 py-1 rounded-full border border-[#E6E3DA] dark:border-[#2A2E29] text-[10px] font-bold text-[#1B4332] dark:text-[#3FA873] tracking-wider uppercase shadow-2xs">
        {badgeText}
      </div>
      ...
      {isOwner && !uploading && (
        <div className="absolute top-3 right-4 sm:top-4 sm:right-6 z-20" ref={menuRef}>
          <button
            ...
            className="h-8 px-3 text-xs font-bold text-[#16160F] dark:text-[#F2F1EC] bg-white/90 dark:bg-[#181B18]/90 hover:bg-white dark:hover:bg-[#202520] backdrop-blur-md border border-[#E6E3DA] dark:border-[#2A2E29] rounded-xl transition-all shadow-sm active:scale-[0.98] cursor-pointer inline-flex items-center gap-1.5"
          >
            <Camera className="w-3.5 h-3.5 text-[#1B4332] dark:text-[#3FA873]" />
            <span>Edit Banner</span>
            <ChevronDown className={`w-3 h-3 text-[#6B6858] dark:text-[#9C9A8C] transition-transform ${menuOpen ? "rotate-180" : ""}`} />
          </button>

          {/* Contextual Dropdown Menu (Opens downward cleanly away from avatar) */}
          {menuOpen && (
            <div className="absolute right-0 top-10 w-44 bg-white dark:bg-[#181B18] border border-[#E6E3DA] dark:border-[#2A2E29] rounded-xl shadow-xl py-1.5 z-30 animate-fadeIn space-y-0.5">
              <button
                type="button"
                onClick={() => {
                  setMenuOpen(false);
              ...
              >
                <ImageIcon className="w-3.5 h-3.5 text-[#1B4332] dark:text-[#3FA873]" />
              ...
              {bannerUrl && (
                <button
                  ...
                  className="w-full px-3.5 py-2 text-xs font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors flex items-center gap-2 cursor-pointer text-left"
                >
