...
export default function MessageInput({
  ...
  return (
    <form
      ...
      className="bg-white dark:bg-[#181B18] border-t border-[#E6E3DA] dark:border-[#2A2E29] p-2.5 sm:p-3 sticky bottom-0 z-20 shrink-0 relative"
    >
      {/* Composer Reply Preview Banner */}
      {replyingTo && (
        <div className="mb-2 px-3 py-2 bg-[#F7F6F2] dark:bg-[#202520] border border-[#E6E3DA] dark:border-[#2A2E29] border-l-4 border-l-[#1B4332] dark:border-l-[#3FA873] rounded-xl flex items-center justify-between gap-2 text-xs animate-in fade-in slide-in-from-bottom-1 duration-150">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 font-bold text-[#1B4332] dark:text-[#3FA873] text-[11px]">
              <Reply className="w-3.5 h-3.5 shrink-0" />
              <span>Replying to {replySenderName}</span>
            </div>
            <p className="text-[11px] text-[#6B6858] dark:text-[#9C9A8C] truncate mt-0.5 italic">
              {replyPreviewText}
            </p>
          </div>
          <button
            ...
            className="p-1 text-[#6B6858] dark:text-[#9C9A8C] hover:text-[#16160F] dark:hover:text-[#F2F1EC] hover:bg-black/5 dark:hover:bg-white/10 rounded-md transition-colors cursor-pointer shrink-0"
      ...
      <div className="w-full flex items-end gap-2 sm:gap-3 relative">
        <div className="flex-1 relative bg-[#F7F6F2] dark:bg-[#202520] border border-[#E6E3DA] dark:border-[#2A2E29] focus-within:border-[#1B4332] dark:focus-within:border-[#3FA873] rounded-2xl transition-all shadow-2xs overflow-hidden flex items-end pr-2">
          <textarea
            ref={textareaRef}
            rows={1}
            value={content}
            ...
            className="w-full px-3.5 py-2.5 text-xs sm:text-sm text-[#16160F] dark:text-[#F2F1EC] placeholder-[#6B6858] dark:placeholder-[#9C9A8C] bg-transparent outline-none border-none ring-0 focus:outline-none focus:ring-0 focus:border-none resize-none max-h-32 disabled:opacity-60"
          />

          {/* Character counter warning when approaching 2000 char limit */}
          {content.length > 1800 && (
            <div className="px-2 pb-2.5 text-[10px] text-amber-700 dark:text-amber-400 font-bold shrink-0 select-none">
              {2000 - content.length}
            </div>
          )}
        </div>
