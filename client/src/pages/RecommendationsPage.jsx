      ...
      <main key={location.pathname} className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-5 sm:py-6 animate-page-enter">
        ...
        <div className="mb-4 sm:mb-5 flex flex-col md:flex-row md:items-end justify-between gap-3 animate-section-enter">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-extrabold text-[#16160F] dark:text-[#F2F1EC] tracking-tight">
              Find Your Skill Match
            </h1>
            <p className="text-xs sm:text-sm text-[#6B6858] dark:text-[#9C9A8C] mt-0.5 max-w-2xl">
              {isAiMode
          ...
          <div className="inline-flex p-0.5 bg-white dark:bg-[#181B18] border border-[#E6E3DA] dark:border-[#2A2E29] rounded-xl shadow-xs">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                isAiMode
                  ? "bg-[#1B4332] dark:bg-[#3FA873] text-white dark:text-[#0F1210] shadow-xs"
                  : "text-[#6B6858] dark:text-[#9C9A8C] hover:text-[#1B4332] dark:hover:text-[#3FA873] hover:bg-[#E4EEE8]/60 dark:hover:bg-[#1C2E24]/60"
              }`}
            >
              <Sparkles className={`w-3 h-3 ${isAiMode ? "text-white dark:text-[#0F1210]" : "text-[#3FA873]"}`} />
        ...
        {isAiMode && !loading && (
          <div className="mb-4 p-2.5 sm:p-3 rounded-xl bg-[#E4EEE8]/70 dark:bg-[#1C2E24]/70 border border-[#1B4332]/20 dark:border-[#3FA873]/30 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
            <div className="flex items-center gap-2">
              <Sparkles className="w-3.5 h-3.5 text-[#3FA873] shrink-0" />
              <span className="font-semibold text-[#1B4332] dark:text-[#3FA873]">
            ...
            <button
              type="button"
              onClick={() => handleModeChange(false)}
              className="text-xs font-bold text-[#1B4332] dark:text-[#3FA873] hover:underline whitespace-nowrap self-start sm:self-auto cursor-pointer"
            >
              ← Back to All Matches
            </button>
          </div>
        ...
        {isAiMode && meta.isAiEnhanced === false && !loading && !error && (
          <div className="mb-4 p-2.5 rounded-xl bg-[#F7F6F2] dark:bg-[#181B18] border border-[#E6E3DA] dark:border-[#2A2E29] flex items-center justify-between gap-2 text-xs text-[#6B6858] dark:text-[#9C9A8C]">
            <div className="flex items-center gap-2">
              <Info className="w-3.5 h-3.5 text-[#6B6858] dark:text-[#9C9A8C] shrink-0" />
              <span>
            ...
            <button
              type="button"
              onClick={() => handleModeChange(false)}
              className="font-bold text-[#1B4332] dark:text-[#3FA873] hover:underline cursor-pointer whitespace-nowrap"
            >
              All Matches
            </button>
          </div>
        ...
        {loading ? (
          <div>
            {isAiMode && (
              <div className="text-center py-2 mb-3">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#E4EEE8] dark:bg-[#1C2E24] text-[#1B4332] dark:text-[#3FA873] text-xs font-bold animate-pulse border border-[#1B4332]/20 dark:border-[#3FA873]/30">
          ...
          isAiMode ? (
            <div className="bg-white dark:bg-[#181B18] border border-[#E6E3DA] dark:border-[#2A2E29] rounded-xl p-6 sm:p-8 text-center max-w-md mx-auto shadow-xs">
              <div className="w-10 h-10 rounded-xl bg-[#E4EEE8] dark:bg-[#1C2E24] text-[#1B4332] dark:text-[#3FA873] flex items-center justify-center mx-auto mb-3">
              ...
              <p className="text-xs text-[#6B6858] dark:text-[#9C9A8C] mt-1 mb-4 leading-relaxed">
                AI couldn't find additional matches right now. Try exploring all matches or adding more skills.
              </p>
              <button
                type="button"
                onClick={() => handleModeChange(false)}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#1B4332] dark:bg-[#3FA873] text-white dark:text-[#0F1210] rounded-lg text-xs font-bold hover:bg-[#143326] dark:hover:bg-[#338d60] transition-colors cursor-pointer shadow-xs"
              >
            ...
            <div className="flex flex-col items-center justify-center pt-1 pb-4">
              {meta.page < meta.totalPages ? (
                ...
                >
                  {loadingMore ? (
                    <>
                      <RefreshCw className="w-3.5 h-3.5 animate-spin text-[#1B4332] dark:text-[#3FA873]" />
                      <span>Loading more matches...</span>
                    </>
                  ) : (
                    <>
                      <span>Load More Matches</span>
                      <ArrowDown className="w-3 h-3 text-[#6B6858] dark:text-[#9C9A8C]" />
                    </>
                  )}
                </button>
              ) : (
                <p className="text-[11px] text-[#6B6858] dark:text-[#9C9A8C] font-medium text-center">
